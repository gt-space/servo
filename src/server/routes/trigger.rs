...
/// Request struct used to delete a trigger from the database and make it inactive on the flight computer.
...
pub async fn delete_trigger(State(shared): State<SharedState>, Json(request): Json<DeleteTriggerRequest>) -> server::Result<()> {
	...
	database
		.execute("DELETE FROM Triggers WHERE name = ?1", params![request.name])
		.map_err(internal)?;
