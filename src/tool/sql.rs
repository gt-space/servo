...
pub async fn sql(sql: &str) -> anyhow::Result<()> {
	...
	if response.rows.len() > 0 {
		let mut column_widths: Vec<usize> = response.column_names
			.iter()
			.map(|name| name.len())
			.collect();
					...
					serde_json::Value::Array(array) => {
						if array.len() <= 8 {
							...
							for val in array {
								let byte = val
									.as_i64()
									.ok_or(anyhow::Error::msg("bad response"))?;
		...
		for (c, name) in response.column_names.iter().enumerate() {
			print!("\x1b[39m|\x1b[33m {name:^width$} ", width = column_widths[c]);
		}
		...
		for row in rows {
			for (c, column) in row.iter().enumerate() {
				print!("\x1b[39;1m|\x1b[22m {column:<width$} ", width = column_widths[c] + 5);
			}
